...
final class AppVideoAnalyzerFixtureProcessLifecycleTests: XCTestCase {
    func testDefaultModeMaySkipAnAbsentRepository() throws {
        let missing = FileManager.default.temporaryDirectory
            .appendingPathComponent("OptionalAppVideoAnalyzer-\(UUID().uuidString)")
        XCTAssertNil(try AppVideoAnalyzerFixtureProcess.repositoryIfAvailable(
    ...
    func testRequiredModeFailsInsteadOfSkippingMissingRepository() throws {
        let missing = FileManager.default.temporaryDirectory
            .appendingPathComponent("MissingAppVideoAnalyzer-\(UUID().uuidString)")
        XCTAssertThrowsError(try AppVideoAnalyzerFixtureProcess.repositoryIfAvailable(
            environment: ["OPENFINDER_REQUIRE_VIDEO_ANALYZER_E2E": "1"],
            defaultRepository: missing
        )) { error in
            XCTAssertEqual(error as? AppVideoAnalyzerFixtureError, .requiredRepositoryMissing)
        }
    }
    ...
    func testIncompleteCheckoutFailsInsteadOfSkippingMissingPython() throws {
        ...
        XCTAssertThrowsError(try AppVideoAnalyzerFixtureProcess.repositoryIfAvailable(
            ...
            XCTAssertEqual(error as? AppVideoAnalyzerFixtureError, .pythonMissing)
        }
    }

    func testReadinessTimeoutReapsOwnedPIDAndRemovesRoot() throws {
        try assertLaunchFailure(mode: "timeout", expected: .readinessTimeout)
    }
    ...
    private func assertLaunchFailure(
        mode: String,
        expected: AppVideoAnalyzerFixtureError,
        file: StaticString = #filePath,
        line: UInt = #line
    ) throws {
        let root = try Self.makeRoot(mode)
        let readiness = root.appendingPathComponent("app-readiness.json")
        let pidURL = root.appendingPathComponent("owned.pid")
        let launch = AppVideoAnalyzerFixtureLaunchOverride(
        ...
        var caught: Error?
        do {
            let fixture = try AppVideoAnalyzerFixtureProcess(
        ...
        XCTAssertEqual(caught as? AppVideoAnalyzerFixtureError, expected, file: file, line: line)
    ...
    private static func makeRoot(_ label: String) throws -> URL {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("AppVideoAnalyzerLifecycle-\(label)-\(UUID().uuidString)", isDirectory: true)
